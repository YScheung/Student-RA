...
import tqdm
from model import GradientUtils, ImageSiren, PixelDataset


img = plt.imread("dog.png")
downsampling_factor = 4 # downsample so to make training faster
img = 2 * ((img) - 0.5)
...
target = "intensity" # What to supervise ("intensity" / "grad" / "laplace")
...
for e in range(n_epochs):
    ...
    if e % logging_freq == 0:
        ...
        for d_batch in tqdm.tqdm(dataloader):
            ...
            pred_img_grad_norm[coords_abs[:,0], coords_abs[:, 1]] = pred_g # prediction with graident as loss
            pred_img_laplace[coords_abs[:,0], coords_abs[:, 1]] = pred_l # prediction with laplace as loss
