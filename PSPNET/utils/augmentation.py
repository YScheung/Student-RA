...
class Compose(object): 
    ...
    def __call__(self, img, anno_class_img):   # Method called when object is created
        for t in self.transforms:
            img, anno_class_img = t(img, anno_class_img)  
        return img, anno_class_img 


# Annotated and Normal image will be scaled together so to ensure they have the same size
class Scale(object): 
    ...
    def __call__(self, img, anno_class_img):
        ...
        scale = np.random.uniform(self.scale[0], self.scale[1]) # Samples are distributed uniformly and an item is randomly drawn
        ...
        if scale > 1.0:
            ...
            p_palette = anno_class_img.copy().getpalette() # .getpalette() returns the image palette as a list.
...
class RandomMirror(object):
    def __call__(self, img, anno_class_img):
        if np.random.randint(2):
            img = ImageOps.mirror(img) # Reflect image. Like the mirror 
...
class Resize(object):
    ...
    def __call__(self, img, anno_class_img):
        img = img.resize((self.input_size, self.input_size), Image.BILINEAR) # Bilinear interpolation in image resizing 
        anno_class_img = anno_class_img.resize((self.input_size, self.input_size), Image.NEAREST) # Nearest neighbour resizing 
...
class Normalize_Tensor(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        index = np.where(anno_class_img == 255) # np.where returns the indices where the condition is satisfied 
